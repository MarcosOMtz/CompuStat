...
# Cambiemos el tamaño de brinco a 0.5, actualmente esta en 0.1
cppFunction('
...
# 3) METROPOLIS ~~> Aquí empieza la grabación
...
burnin <- 100 # Eliminar las pimeras 100 observaciones
estim <- estims[-(1:burnin), ]
thinning <- 2 # meaning we'll keep 75% of observations to reduce autocorrelation ~~> original 0.9
              # El adelgazamiento normalmente puede ser de 2 en 2
seq(1, nsim-burning, by=thinning)
estim <- estim[pts,]


# OBS: thinning is rarely usefull!!!! check that nothing changes ~~> Esto se puede borrar
...
#intmeanlow <- c(intervals[1], intervals2[1], intervals3[1]) #~~> Original
#intmeanhigh <- c(intervals[2], intervals2[2], intervals3[2]) #~~> Original
intmeanlow <- c(intervals[1], NA, intervals3[1])
intmeanhigh <- c(intervals[2], NA, intervals3[2])
